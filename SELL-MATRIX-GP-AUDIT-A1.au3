;******************************************************************************
; NAME: ECLIPSE SELL MATRIX GP AUDIT REPORT CONVERTER
; FILENAME: SELL-MATRIX-GP-AUDIT-XX.AU3
; REVISION: 1.0
;
; TYPE: AUTOIT    VERSION:   V 3.3.12.0        DATE: 2018:03:08
;******************************************************************************
; PROGRAM DESCRIPTION:
;
...
;
;******************************************************************************
; SELL MATRIX GP% AUDIT REPORT PARAMETERS:
;
; BRANCH / TERRITORY: 1
...
; CUSTOMER TYPE:
; PRICE LINE: 3M
; PRICE GROUP: 36322925
; COMPARE TO: COMM-COST
; CATALOG ITEMS: EXCLUDE
; SHOW ITEMS WITH GP%:
;
;***********************************************************************************
;------------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------------
;001	A		BASIS								$sA1  LINE1			50		7
;002	B		FORMULA								$sB1  LINE1			58		6
;003	C		PRODUCT DESCRIPTION					$sC1  LINE2			01		35
;004	D		PRICE LINE							$sD1  LINE2			39		10
;005	E		GM%									$sE1  LINE2			58		6
;006	F		HITS								$sF1  LINE2			65		6
;***********************************************************************************
;***********************************************************************************
...
$mLbTitle = GUICtrlCreateLabel("SELL MATRIX AUDIT REPORT CONVERSION", 24, 16, 300, 17)
...
Func RWFile($sFilePath, $dFilePath)
	...
	;Write Header
	$sWrt = "DESCRIPTION" & @TAB
	$sWrt = $sWrt & "PRICELINE" & @TAB
	$sWrt = $sWrt & "BASIS" & @TAB
	$sWrt = $sWrt & "FORMULA" & @TAB
	$sWrt = $sWrt & "GM-PCT" & @TAB
	$sWrt = $sWrt & "HITS"
	...
	$i = 0
	While $i <= $iEnd
		$sMrk1 = StringMid ($aArray[$i],5 ,16) ; End of header lines
		If $sMrk1 == "Product Specific" Then
			$iStart = $i
			ExitLoop
		EndIf
		$i = $i + 1
	...
	For $i = $iStart to $iEnd
		...
		$sMrk2 = StringMid ($aArray[$i],5 ,16) ; "Product Specific"
		...
		If $sMrk2 == "Product Specific" Then

			;001	A		BASIS								$sA1  LINE1			50		7
			$sA1 = StringMid ($aArray[$i],50 ,7)
